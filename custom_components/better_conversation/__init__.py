...
STORAGE_KEY = f"{DOMAIN}.conversation_history"
STORAGE_VERSION = 1
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    await agent.async_load_history()
...
class OpenAIAgent(conversation.AbstractConversationAgent):
    ...
    async def async_load_history(self) -> None:
        ...
        try:
            ...
            if stored_data and "history" in stored_data:
                ...
                _LOGGER.info(
                    "Loaded %d conversation histories from storage", len(self.history)
                )
        except Exception as err:
            _LOGGER.error("Failed to load conversation history: %s", err)
    ...
    async def async_save_history(self) -> None:
        ...
        try:
            ...
            _LOGGER.debug(
                "Saved %d conversation histories to storage", len(self.history)
            )
        except Exception as err:
            _LOGGER.error("Failed to save conversation history: %s", err)
    ...
    async def async_cleanup_old_conversations(
        ...
        conversation_keys = list(self.history.keys())
        conversations_to_remove = conversation_keys[:-max_conversations]

        for conv_key in conversations_to_remove:
            del self.history[conv_key]
            # Also clean up the mapping if it exists
            if hasattr(self, "_conversation_mapping"):
                self._conversation_mapping.pop(conv_key, None)

        await self.async_save_history()
        _LOGGER.info(
            "Cleaned up %d old conversations, kept %d",
            len(conversations_to_remove),
            len(self.history),
        )
    ...
    async def async_process(
        ...
        # Create a unique conversation key based on device_id and user_id
        # This ensures conversations are properly tracked within the same session
        conversation_key = self._get_conversation_key(user_input)

        if conversation_key in self.history:
            conversation_id = conversation_key
            messages = self.history[conversation_key]

            # Check if we need to refresh the system message due to configuration changes
            self._refresh_system_message_if_needed(
                messages, exposed_entities, user_input
            )
        else:
            conversation_id = ulid.ulid()
            # Store the mapping from conversation_key to conversation_id
            if not hasattr(self, "_conversation_mapping"):
                self._conversation_mapping = {}
            self._conversation_mapping[conversation_key] = conversation_id
        ...
        self.history[conversation_key] = messages

        # Save conversation history to storage
        await self.async_save_history()

        # Cleanup old conversations if needed
        max_conversations = self.entry.options.get(
            CONF_MAX_CONVERSATIONS, DEFAULT_MAX_CONVERSATIONS
        )
        await self.async_cleanup_old_conversations(max_conversations)
    ...
    def _get_conversation_key(self, user_input: conversation.ConversationInput) -> str:
        """Generate a unique conversation key based on device and user."""
        device_id = user_input.device_id or "unknown_device"
        user_id = user_input.context.user_id or "unknown_user"
        conversation_key = f"{device_id}_{user_id}"

        # Add debug logging to help troubleshoot conversation tracking
        _LOGGER.debug(
            "Conversation key: %s (device_id: %s, user_id: %s, original_conversation_id: %s)",
            conversation_key,
    ...
    def _generate_system_message(
        ...
        # Add debug logging to show which prompt is being used
        _LOGGER.debug(
            "Using prompt template: %s",
            raw_prompt[:100] + "..." if len(raw_prompt) > 100 else raw_prompt,
        )

        return {"role": "system", "content": prompt}

    def _should_refresh_system_message(self, messages) -> bool:
        """Check if the system message should be refreshed based on current configuration."""
        if not messages or messages[0]["role"] != "system":
            return True

        current_prompt = self.entry.options.get(CONF_PROMPT, DEFAULT_PROMPT)
        stored_content = messages[0]["content"]

        # Check if the stored system message contains the current prompt template
        # This is a simple check - if the prompt template changed, refresh
        return current_prompt not in stored_content

    def _refresh_system_message_if_needed(
        self, messages, exposed_entities, user_input: conversation.ConversationInput
    ):
        """Refresh the system message if the configuration has changed."""
        if self._should_refresh_system_message(messages):
            _LOGGER.info("Refreshing system message due to configuration change")
